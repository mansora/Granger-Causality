function [SignificanceMap, SM]=PermutationTest_MCP_perCluster(Group1, Group2, Iterations, tthresh)
%% Permutation test with cluster based inference for multiple comparison for within and between subject analysis of time-frequency plots
    ...
    sigMap_Observed_bin=double(sigMap_Observed>tthresh);
    sigMap_Observed(sigMap_Observed<tthresh)=0;
    
    
    MCP_obs=zeros(varr,varr);
    for i=1:varr
        for j=1:varr
            sigMap_Observed_bin_temp=squeeze(sigMap_Observed_bin(i,j,:,:));
            sigMap_Observed_temp=squeeze(sigMap_Observed(i,j,:,:));
            xtemp=bwconncomp(sigMap_Observed_bin_temp);    
    ...
    for It=1:Iterations
        ...
        sigMap_temp_bin=double(sigMap_temp>tthresh);
        sigMap_temp(sigMap_temp<tthresh)=0;
        
        MCP_perm=zeros(varr,varr);
        for i=1:varr
            for j=1:varr
                sigMap_temp_temp_bin=squeeze(sigMap_temp_bin(i,j,:,:));
                sigMap_temp_temp=squeeze(sigMap_temp(i,j,:,:));
                xtemp=bwconncomp(sigMap_temp_temp_bin); 
                clusterStats=zeros(1,xtemp.NumObjects);
                if xtemp.NumObjects~=0
                    for k=1:xtemp.NumObjects
