function [siglev_Mode1, siglev_Mode2]=arres_whiteness(res_all,p,k)
...
%        Models, J. Roy. Stat. Soc. B, 43, 231--239.  
  ...
  [m,n,trials]= size(res_all);                    % dimension of state vectors
  randomtrials=randperm(trials);
  for tt=1:20
      res=squeeze(res_all(:,:,randomtrials(tt)));
      res=res';
      res=res(p+1:end,:);
    ...
    %   % end of loop over l
      ...
      for ll=1:k
        cl  = (res(1:nres-ll, :)'*res(ll+1:nres,:))./dd;  % lag l correlation matrix
      ...
      siglev_tt_Mode1(tt) = 1 - gammainc(lmp/2, dof_lmp/2);
      siglev_tt_Mode2(tt) = 1 - chi2pdf(lmp/2, dof_lmp/2);

  end
  siglev_Mode1=sum(siglev_tt_Mode1);
  siglev_Mode2=sum(siglev_tt_Mode2);
